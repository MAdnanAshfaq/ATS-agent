...
[![Gemini 2.5 / 3.5](https://img.shields.io/badge/Gemini_API-2.5_%2F_3.5_Flash-4285F4?style=for-the-badge&logo=google&logoColor=white)](https://aistudio.google.com/)
...
[![Zero Hallucinations](https://img.shields.io/badge/Rule_0-Truth_Grounded-9B51E0?style=for-the-badge)](#rule-0-candidate-truth-grounding)
[![Anti-AI Human Voice](https://img.shields.io/badge/Anti--AI-CV_%E2%89%A5_0.25-FF5722?style=for-the-badge)](#mathematical-anti-ai-rhythm--human-voice-audit)
...
```mermaid
graph TD
    JD[Raw Job Description] --> R[Role 1: Researcher]
    R -->|Atomic Rubric: Hard/Soft Skills & Keywords| W[Role 2: Targeted Writer]
    Base[Master Resume JSON] --> W
    W -->|Draft Tailored Resume| A[Role 3: Forensic Auditor]
    A -->|Evaluates Claim Provenance & Human Voice| C{Passes Gated Audit?}
    C -- No --> E[Role 4: Executive Editor]
    E -->|Corrected Draft| A
    C -- Yes --> P[docx_patcher.py & Word COM PDF Engine]
```
...
sequenceDiagram
    ...
    participant UI as Flask Web UI & HollaBuddy
    participant Scraper as Playwright Scraper
    participant Matcher as Keyword Matcher
    participant Dani as Dani's Multi-Agent Engine
    participant Patcher as docx_patcher.py
    participant COM as Word COM / PDF Engine

    User->>UI: Pastes Job URL (or enters manual JD)
    UI->>Scraper: Scrape target job page
    Scraper-->>UI: Clean JD text, role title, company name
    UI->>Matcher: Compare JD against base_resume.json
    Matcher-->>UI: Matched keywords, missing keywords, ATS score
    
    rect rgb(240, 248, 255)
        Note over Dani: Dani's 4-Role Tailoring Loop
        UI->>Dani: Run Researcher Phase (Rubric extraction)
        Dani->>Dani: Run Targeted Writer Phase (Rules 0-16)
        Dani->>Dani: Run Auditor Phase (Truth Grounding & CV >= 0.25)
        Dani->>Dani: Run Editor Phase (Fix any audit failures)
    end
    
    Dani-->>UI: Tailored Resume Data & Cover Letter
    UI->>Patcher: Patch master_resume_original.docx in-place
    Patcher-->>COM: Save tailored .docx file
    COM-->>UI: Export high-fidelity vector .pdf
    UI->>User: Display preview modal & download links
```
